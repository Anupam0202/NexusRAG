...
class CacheFence:
    workspace_id:str;authorization_revision:int;workspace_policy_hash:str;rights_hash:str;source_version_hashes:tuple[str,...];operation:str;prompt_version:str;model_revision:str;parameters:tuple[tuple[str,str],...]=()
    def __post_init__(self)->None:
        ...
        for digest in (self.workspace_policy_hash,self.rights_hash,*self.source_version_hashes):
            if len(digest)!=64 or any(c not in "0123456789abcdef" for c in digest.lower()):raise ValueError("Cache fence hashes must be SHA-256 hex")
