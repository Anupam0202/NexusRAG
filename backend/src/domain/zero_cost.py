...
class ResourceBudget:
    ...
    def __post_init__(self) -> None:
        if self.used < 0 or self.reserved < 0:
            raise ValueError("Usage and reservation values must be non-negative")
        if self.hard_limit is not None and self.hard_limit < 0:
            raise ValueError("Hard limit must be non-negative")
...
def admit(
    ...
    *,
    ...
    """Make a deterministic, no-paid-fallback admission decision."""
    if amount <= 0:
        raise ValueError("Reservation amount must be positive")
    if rights_review_required:
        return _deny(budget, CapacityState.REVIEW_REQUIRED, "Rights require review")
    if not rights_allowed:
        return _deny(budget, CapacityState.RIGHTS_BLOCKED, "Rights policy denies operation")
    if budget.provider_paused:
        return _deny(budget, CapacityState.PROVIDER_PAUSED, "Provider is paused")
    if not budget.provider_available:
        return _deny(budget, CapacityState.PROVIDER_UNAVAILABLE, "Provider is unavailable")
    if budget.read_only:
        return _deny(budget, CapacityState.READ_ONLY, "Resource is read-only")
    if budget.hard_limit is None:
        return _deny(budget, CapacityState.REVIEW_REQUIRED, "Quota is undocumented")
    ...
    if ratio >= 0.95 and priority not in {Priority.ESSENTIAL}:
        return AdmissionDecision(False, CapacityState.CAPACITY_REACHED, "Capacity reserved for essential export and deletion", ratio, remaining, budget.reset_at)
    if ratio >= 0.85 and priority in {Priority.BACKGROUND, Priority.SPECULATIVE}:
        return AdmissionDecision(False, CapacityState.DEGRADED, "Nonessential work is delayed above 85% utilization", ratio, remaining, budget.reset_at)
    if ratio >= 0.70 and priority is Priority.SPECULATIVE:
        return AdmissionDecision(False, CapacityState.DEGRADED, "Speculative work is disabled above 70% utilization", ratio, remaining, budget.reset_at)
...
def admit_all(budgets: Mapping[str, ResourceBudget], requested: Mapping[str, int], *, priority: Priority = Priority.INTERACTIVE) -> dict[str, AdmissionDecision]:
    ...
    if unknown:
        raise ValueError(f"Missing budgets: {sorted(unknown)}")
