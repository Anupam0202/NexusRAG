...
class ReservationState(StrEnum): RESERVED="RESERVED";SETTLED="SETTLED";RELEASED="RELEASED";RECONCILIATION_REQUIRED="RECONCILIATION_REQUIRED"
...
class ReservationLedger:
    def __init__(self)->None:
        self._lock=RLock();self._budgets={};self._reservations={};self._keys={}
    ...
    def reserve(self,*,workspace_id:str,provider:str,resource:str,amount:int,idempotency_key:str,priority:Priority=Priority.INTERACTIVE)->tuple[Reservation|None,dict[str,AdmissionDecision]]:
        if not all((workspace_id,provider,resource,idempotency_key)): raise ValueError("Reservation identity is incomplete")
        with self._lock:
            ...
            if key in self._keys:
                ...
                return existing,{}
            dims={"global":self._require_budget("global",provider,resource),"workspace":self._require_budget(f"workspace:{workspace_id}",provider,resource)}
            decisions={scope:admit(budget,amount,priority=priority) for scope,budget in dims.items()}
            if not all(x.allowed for x in decisions.values()): return None,decisions
            ...
            for scope,budget in dims.items():
