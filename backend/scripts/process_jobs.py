...
from src.infrastructure.supabase_client import get_supabase_client  # noqa: E402
...
async def process_one(*, workspace_id: str | None = None) -> bool:
    settings = get_settings()
    jobs = IngestionJobRepository()
    documents = DocumentRepository()
    supabase = get_supabase_client()

    job = await jobs.claim_next_queued(workspace_id=workspace_id)
    if not job:
        return False
    ...
    content = await supabase.download_object(storage_path)
    user = CurrentUser(
...
async def main() -> None:
    ...
    while args.poll or processed < args.limit:
        did_work = await process_one(workspace_id=args.workspace_id)
        if did_work:
            processed += 1
            continue
        if not args.poll:
