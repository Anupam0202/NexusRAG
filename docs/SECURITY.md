...
- Supabase Storage and metadata tables are authoritative; Cloudflare local state and Qdrant vectors are not customer-record authorities.
...
- Cloudflare frontend builds receive only `NEXT_PUBLIC_*` values; provider and service credentials remain Worker or GitHub Environment secrets.
- Cloudflare gateway secrets and GitHub Preview secrets are scoped separately from public frontend variables.
- Do not enable anonymous demo mode in production.
- Store Google and GitHub client credentials only in Supabase Auth provider settings, never in Cloudflare public variables, browser bundles, or the repository.
...
2. Rotate Cloudflare, Supabase, Qdrant, and Gemini credentials.
