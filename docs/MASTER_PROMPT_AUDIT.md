...
The candidate is in draft PR #3 on an isolated validation branch, targeting the feature branch rather than `main` or the named Preview deployment branch. The latest completed attempts failed before migration SQL because public container registries returned `toomanyrequests` (first GHCR; an authenticated retry still failed, then the public ECR pull was also rate-limited). CI was changed to install PostgreSQL 17 and pgvector from the public PostgreSQL APT repository and run the committed clean-baseline, migration, Storage-policy-stand-in, batching, and independent-concurrency harness directly against PostgreSQL; see [PostgreSQL APT](https://wiki.postgresql.org/wiki/Apt) and [pgvector](https://github.com/pgvector/pgvector). Its first run exposed that the runner did not auto-create a PostgreSQL 17 cluster; the workflow now explicitly creates and starts one on an isolated port before testing. The same harness passed locally on PostgreSQL 17.10. This validates PostgreSQL behavior, not Supabase Auth/Storage services or hosted project behavior. CI does not deploy Cloudflare or call live providers. Do not merge yet: hosted Supabase/Auth behavior, Gemini plan/rights/privacy/budget, candidate runtime, authenticated product E2E, semantic quality, and product/release gates remain open.
