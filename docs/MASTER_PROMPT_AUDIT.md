...
The candidate is now in draft PR #3 on an isolated validation branch with 9/9 GitHub checks passing. The PR targets the existing feature branch, not `main` or the named Preview deployment branch. CI ran the local Supabase stack and did not run Cloudflare deploy or live-provider workflows. Do not merge yet: hosted Supabase, Gemini privacy/rights/budget, candidate runtime, authenticated assurance, semantic quality, and product/release gates remain open.
