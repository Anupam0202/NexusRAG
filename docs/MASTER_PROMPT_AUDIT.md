...
The candidate is in draft PR #3 on an isolated validation branch, targeting the feature branch rather than `main` or the named Preview deployment branch. At the latest completed run before the current CI fix, 8/9 checks passed; the Supabase local-stack job failed before SQL because GHCR returned `toomanyrequests` while pulling `ghcr.io/supabase/postgres:17.6.1.167`. This is a registry pull failure, not a migration assertion failure. The complete clean baseline plus migrations 027/028 was independently rerun in a disposable local PostgreSQL 17.10 database with pgvector; reservation concurrency, idempotency, settlement replay, storage-policy stand-ins and batch staging assertions all passed. The CI workflow now authenticates GHCR image pulls with the ephemeral job token and retries service startup up to three times; its new run is pending. CI does not deploy Cloudflare or call live providers. Do not merge yet: hosted Supabase/Auth behavior, Gemini plan/rights/privacy/budget, candidate runtime, authenticated product E2E, semantic quality, and product/release gates remain open.
