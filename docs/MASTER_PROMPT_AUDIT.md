...
The candidate is in draft PR #3 on an isolated validation branch, targeting the feature branch rather than `main` or the named Preview deployment branch. The latest two completed workflow attempts both passed 8/9 checks; the Supabase local-stack job failed before SQL because GHCR returned `toomanyrequests` pulling `ghcr.io/supabase/postgres:17.6.1.167`. Authenticated GHCR pulls and bounded retries did not clear the registry throttle. The exact image tag was verified in the official [Amazon ECR Public Gallery mirror](https://gallery.ecr.aws/supabase/postgres), including Linux/amd64 support. The CI workflow now pre-pulls that public image and tags it locally under the GHCR name expected by Supabase CLI; the new run is pending. Independently, the complete clean baseline plus migrations 027/028 passed again in a disposable local PostgreSQL 17.10 database with pgvector; reservation concurrency, idempotency, settlement replay, storage-policy stand-ins and batch staging assertions all passed. CI does not deploy Cloudflare or call live providers. Do not merge yet: hosted Supabase/Auth behavior, Gemini plan/rights/privacy/budget, candidate runtime, authenticated product E2E, semantic quality, and product/release gates remain open.
