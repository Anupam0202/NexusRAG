...
## Google OAuth callback/session verified; no product data created — 2026-09-24

A live browser check of the isolated candidate frontend completed the Google OAuth redirect through the rehearsal Supabase project, processed the callback, established an authenticated app session, and reached the protected onboarding page. This closes provider initiation/callback/session plumbing for one Google identity only. The identity came from the browser's existing Google SSO session, not an owner-designated test account; do not count this as two-user or ownership verification. The app displayed an authenticated account, and the browser session was signed out immediately. No workspace was created, and no document, chat message, Gemini call, Qdrant point, or Storage object was created. A rehearsal Auth user record may have been created by OAuth; confirm that the account is an approved test identity and clean up that Auth record before considering the fixture reset complete. Do not disclose or assume ownership of the browser account.

On the unauthenticated candidate `/chat` route, UI controls remain disabled until sign-in, as expected; the header currently labels the backend `Backend offline` because the unauthenticated `/api/v1/status` request returns 401. Determine whether this is an auth-gated health-display defect during authenticated app testing. A live axe-core scan found zero violations and 35 passes; one serious-impact color-contrast check on four gradient-text targets was incomplete because axe could not calculate the gradient backgrounds. This requires manual contrast verification and is not a clean accessibility sign-off.
...
After the Google callback, the isolated rehearsal project had one Auth user and no workspace/document/Storage data. The OAuth browser session was signed out. The test-only Auth row and linked identity were subsequently removed from this disposable project. Post-cleanup read-only counts confirm zero Auth users, identities, workspaces, documents, and Storage objects. No production data was touched.
...
**No production write, app data creation, Storage write, vector insertion, or Gemini call occurred.** OAuth fixture cleanup is complete; post-cleanup counts are zero.
...
Code commit `211ed551f313c182b284cac6f4cd66353652200f` was candidate-deployed in run #42. All 11 reported checks passed, including frontend lint/unit/typecheck/build, PostgreSQL 17 rehearsal, gateway deployment, frontend deployment and desktop/mobile Playwright. A live signed-out candidate browser inspection showed the expected `Sign in required` text, with chat/upload controls gated until sign-in. The browser session was signed out; the test-only Auth row was removed and read-only counts confirm zero Auth users/identities, workspaces, documents, and Storage objects. No provider call or app data was created.

PR #3 code head `211ed551f313c182b284cac6f4cd66353652200f` is GitHub-mergeable but remains draft; this does not close the substantive zero-spend, two-identity authenticated E2E, semantic quality, contrast/accessibility, or release/recovery gates. PR #2 remains blocked by the protected mainline `Preview Required` deployment and required branch conditions. Do not merge or deploy production while those blockers remain.


## Production original-object uniqueness reconciliation — 2026-09-25

A deeper read-only review corrects the earlier 2026-09-24 interpretation of the production schema. Migration `026_allow_reindex_original_reuse.sql` explicitly drops `document_versions_original_bucket_original_key_key` and creates the non-unique `document_versions_original_object_idx`; its comment says reindexing intentionally creates multiple versions over the same immutable source object. Production is at migration 026, so absence of that unique constraint is the designed state, not a production schema defect.

The count-only duplicate group was inspected without reading object bucket/key values or document content. It contains two versions in the same workspace and document, with the same source hash and byte size. The older version is inactive and has no chunks; the newer version is active and has one chunk. Both have an ingestion-job reference and neither has a query-source reference. This is consistent with the documented reindex/reuse lifecycle. No row, source object, or production schema was changed. The previous recommendation to deduplicate this group or restore the unique constraint is withdrawn; no owner-approved duplicate remediation is needed.

The clean rehearsal harness had applied the reviewed baseline and candidate migrations 027–033 but skipped migration 026, creating the apparent schema mismatch. Commit `27cd914a0eac53027d1c59dc4fa678ecd8a68fbf` corrects the harness to apply 026 after the baseline and adds a two-version/same-source fixture plus an assertion that the lookup index is non-unique. The workflow passed on this commit (all 9 reported checks, including PostgreSQL 17 and concurrency). This is a test-harness correction, not a production migration. The production restore/forward-change gate remains until migrations 027–033, runtime bindings, and a recovery plan are reviewed; no production deployment is authorized by this correction.

The prior OAuth test fixture has been removed from the separate Free rehearsal project. Verification found zero Auth users, identities, workspaces, documents, and Storage objects. This closes cleanup only, not two-identity authenticated E2E.


## Reindex rehearsal parity verified — 2026-09-25

The isolated `$0/month` Supabase rehearsal project was brought into line with the production migration-026 reindex rule using an idempotent test-project-only migration named `rehearsal_026_reindex_original_reuse_compat`. Post-change catalog checks on both production and rehearsal confirm the original-object unique constraint is absent and `document_versions_original_object_idx` is non-unique. The PostgreSQL 17 CI fixture verifies two versions can reference the same immutable source address after migration 026. No production DDL or row changes were made.

This closes the earlier claimed production duplicate-key/schema mismatch: the observed duplicate version group is valid under migration 026 and does not need deletion or deduplication. Production still has not received candidate migrations 027–033, so production backup/restore, authorized staged rollout, app/runtime reconciliation, protected-preview gates, and the other release blockers remain open.
