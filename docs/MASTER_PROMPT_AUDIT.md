...
- Migrations 032 and 033 were applied only to the already-existing `$0/month` disposable `nexusrag-zero-cost-rehearsal` project, not to `fcjaomiceajcdownarel`. Hosted PostgreSQL assertions passed for the 5/6 chat boundary, first/additional/10th/11th document cases, per-account isolation, BYOK unlock, idempotent replay, and transaction rollback. Eight independent simultaneous hosted SQL calls against a fresh synthetic account produced exactly five `READY` and three `BYOK_REQUIRED` results.
...
- Validation on the current local candidate: Cloudflare suite **51/51**, frontend **79/79**, frontend lint passed, TypeScript passed, and Cloudflare/migration source checks passed. Hosted direct SQL checks are not a substitute for the full CI clean-baseline rehearsal, which is wired into the PR workflow.
