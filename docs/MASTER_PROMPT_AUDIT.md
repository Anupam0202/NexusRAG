...
The clean rehearsal harness applied the reviewed baseline and candidate migrations 027–033 but skipped migration 026, which created the apparent schema mismatch. The harness is being corrected to apply 026 between the baseline and 027, and to exercise two versions referencing the same immutable source key while asserting that the replacement index is non-unique. This is a test-harness correction, not a production migration. Keep the production restore/forward-change gate until the full candidate migration set, current runtime bindings, and recovery plan are reviewed; no production deployment is authorized by this correction.
