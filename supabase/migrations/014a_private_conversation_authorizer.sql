 ...
 select exists(
  ...
  where s.workspace_id=p_workspace and s.id=p_session and s.deleted_at is null
   ...
   and (s.user_id=auth.uid() or s.visibility='legacy_workspace' or exists(
    ...
    where p.workspace_id=s.workspace_id and p.session_id=s.id and p.user_id=auth.uid()
   ))
 )
$$;
...
create policy chat_sessions_read_authorized on public.chat_sessions for select to authenticated using(nexusrag_private.can_read_conversation(workspace_id,id));
create policy chat_messages_read_authorized on public.chat_messages for select to authenticated using(nexusrag_private.can_read_conversation(workspace_id,session_id));
