...
async function deleteQdrantDocument(env, workspaceId, documentId) {
  if (!env.QDRANT_URL || !env.QDRANT_API_KEY) throw error("CONFIGURATION_ERROR", "Qdrant is not configured.", 503); const collection = env.QDRANT_COLLECTION || "nexusrag-v6-preview"; const root = `${env.QDRANT_URL.replace(/\/$/, "")}/collections/${encodeURIComponent(collection)}`; const headers = { "content-type": "application/json", "api-key": env.QDRANT_API_KEY }; const filter = { must: [{ key: "workspace_id", match: { value: workspaceId } }, { key: "document_id", match: { value: documentId } }] };
  const response = await fetch(`${root}/points/delete?wait=true`, { method: "POST", headers, body: JSON.stringify({ filter }), signal: AbortSignal.timeout(20_000) }); if (!response.ok) throw error("QDRANT_DELETE_FAILED", "Qdrant deletion failed.", 503, true);
  const verification = await fetch(`${root}/points/count`, { method: "POST", headers, body: JSON.stringify({ exact: true, filter }), signal: AbortSignal.timeout(20_000) }); if (!verification.ok || Number((await verification.json())?.result?.count || 0) !== 0) throw error("QDRANT_DELETE_UNVERIFIED", "Qdrant deletion could not be verified.", 503, true); return { verified: true };
}
export { deleteQdrantDocument, extractFileText, hybridFuse, lexicalScore, unzipEntries };
