...
def require_fragments(migration_name: str, fragments: tuple[str, ...]) -> None:
    text = (MIGRATIONS / migration_name).read_text(encoding="utf-8").casefold()
    for fragment in fragments:
        if fragment not in text:
            fail(f"{migration_name} missing contract fragment: {fragment}")
